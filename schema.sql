-- Angry Baseball 後端 D1 結構（全球排行榜）
-- 套用：wrangler d1 execute angry-baseball-db --file=./schema.sql --remote
...
CREATE TABLE IF NOT EXISTS scores (
  ...
  note TEXT,          -- 額外資訊（不影響排名）：大賽＝全壘打數；生存＝安打數；最遠＝模式
  created_at INTEGER
);
...
-- 全服累計統計（單列 id=1）：遊玩場次 / 全壘打數 / 遊玩秒數
CREATE TABLE IF NOT EXISTS stats (
  id INTEGER PRIMARY KEY,
  plays INTEGER NOT NULL DEFAULT 0,
  kills INTEGER NOT NULL DEFAULT 0,   -- 沿用欄位名：這裡記「全壘打數」
